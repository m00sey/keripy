...
def test_withness_receiptor(seeder):
    with habbing.openHby(salt=coring.Salter(raw=b'wann-the-witness').qb64, name="wan") as wanHby, \
            habbing.openHby(salt=coring.Salter(raw=b'will-the-witness').qb64, name="wil") as wilHby, \
            habbing.openHby(salt=coring.Salter(raw=b'wess-the-witness').qb64, name="wes") as wesHby, \
...
def test_witness_sender(seeder):
    with habbing.openHby(salt=coring.Salter(raw=b'wann-the-witness').qb64, name="wan") as wanHby, \
            habbing.openHby(salt=coring.Salter(raw=b'will-the-witness').qb64, name="wil") as wilHby, \
            habbing.openHby(salt=coring.Salter(raw=b'wess-the-witness').qb64, name="wes") as wesHby, \
            habbing.openHby(salt=coring.Salter(raw=b'0123456789abcdef', name="pal").qb64) as palHby:
...
def test_witness_inquisitor(mockHelpingNowUTC, seeder):
    with habbing.openHby(salt=coring.Salter(raw=b'wann-the-witness').qb64, name="wan") as wanHby, \
            ...
            habbing.openHby(salt=coring.Salter(raw=b'wess-the-witness').qb64, name="wes") as wesHby, \
            habbing.openHby(salt=coring.Salter(raw=b'0123456789abcdef').qb64, name="pal") as palHby, \
            habbing.openHby(salt=coring.Salter(raw=b'abcdef0123456789').qb64, name="qin") as qinHby:
        wanDoers = indirecting.setupWitness(alias="wan", hby=wanHby, tcpPort=5632, httpPort=5642)
        wilDoers = indirecting.setupWitness(alias="wil", hby=wilHby, tcpPort=5633, httpPort=5643)
        wesDoers = indirecting.setupWitness(alias="wes", hby=wesHby, tcpPort=5634, httpPort=5644)
